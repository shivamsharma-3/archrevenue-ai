...
export default function AdminDashboard() {
  ...
  // Derived Metrics
  const { mrr, activeSubs, recentSignups } = useMemo(() => {
    let proCount = 0;
    users.forEach(u => {
  ...
  const handleMakePro = async (targetUid: string) => {
    ...
    try {
      ...
      await logSystemEvent('User Upgraded to Pro', 'success', targetUid);
    } catch (e: any) {
      console.error(e);
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {loadingUsers ? (
                          <tr>
                           ...
                           <tr>
                            <td colSpan={4} className="px-6 py-12 text-center text-gray-500 text-[14px]">No users found in database.</td>
                          </tr>
                        ) : (
                          users.map(user => (
                              ...
                              <td className="px-6 py-4 text-right">
                                {user.role !== 'admin' && user.role !== 'pro' && (
                                  <button
                                    ...
                                    className="inline-flex items-center px-4 py-2 text-[12px] font-bold text-white bg-gray-900 hover:bg-gray-800 rounded-lg shadow-sm transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100"
                                  >
